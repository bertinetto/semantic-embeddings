...
if __name__ == '__main__':
    ...
    arggroup.add_argument('--optimizer', type=str, default='adam', help='optimizer to use')
    arggroup.add_argument('--adam_lr', type=float, default=1e-5, help='Learning rate used by Adam optimizer')
    ...
    arggroup.add_argument('--cls_weight', type=float, default=0.0,
                          help='If set to a positive value, an additional classification layer will be added and this parameter specifies the weight of the softmax loss.')
    arggroup.add_argument('--cls_base', type=str, default=None,
                          help='Name or index of the layer that the classification layer should be based on. If not specified, the final embedding layer will be used.')
    arggroup.add_argument('--lr_schedule', type=str, default='adam-no-schedule', choices=utils.LR_SCHEDULES, help='Type of learning rate schedule.')
    ...
    arggroup.add_argument('--snapshot_best', type=str, nargs='?', default=None, const='val_loss',
                          help='Only store best-performing model as checkpoint, identified by monitoring the specified metric.')
    ...
    arggroup.add_argument('--weight_dump', type=str, default=None,
                          help='Filename where the learned model weights should be written to (without model definition).')
    ...
    callbacks, num_epochs = utils.get_lr_schedule(args.lr_schedule, data_generator.num_train, args.batch_size, args.optimizer, args.adam_lr, args.epochs,
                                                  schedule_args={arg_name: arg_val for arg_name, arg_val in vars(args).items() if arg_val is not None})

    if args.optimizer == 'paper_default':
        optimizer = keras.optimizers.SGD(lr=args.sgd_lr, decay=0.0, momentum=0.9, nesterov=args.nesterov, clipnorm=args.clipgrad)
    elif args.optimizer == 'adam':
        optimizer = keras.optimizers.Adam(lr=args.adam_lr, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=True)
    else:
        raise ValueError('Unknown choice of optimizer', args.optimizer)
    ...
    if args.loss.endswith('_corr'):
        ...
        if len(args.top_k_acc) > 0:
            for k in args.top_k_acc:
                metrics.append(
                    utils.top_k_acc(k) if (args.loss == 'softmax_corr') or (args.embedding == 'onehot') else utils.nn_accuracy(embedding, dot_prod_sim=True,
                                                                                                                               k=k))
    else:
    ...
    if args.finetune:
        ...
        if args.finetune_init > 0:
            ...
            if args.cls_weight > 0:
                par_model.compile(optimizer=optimizer,
            ...
            par_model.fit_generator(
                ...
                validation_data=data_generator.test_sequence(args.val_batch_size, batch_transform=transform_inputs,
    ...
    if args.snapshot:
        ...
        callbacks.append(
            keras.callbacks.ModelCheckpoint(args.snapshot, **snapshot_kwargs) if args.gpus <= 1 else utils.TemplateModelCheckpoint(model, args.snapshot,
                                                                                                                                   **snapshot_kwargs))
    ...
    print(par_model.evaluate_generator(
        data_generator.test_sequence(args.val_batch_size, batch_transform=transform_inputs, batch_transform_kwargs=batch_transform_kwargs)))
    if args.cls_weight > 0:
        test_pred = par_model.predict_generator(
            data_generator.test_sequence(args.val_batch_size, batch_transform=transform_inputs, batch_transform_kwargs=batch_transform_kwargs))[1].argmax(
            axis=-1)
        class_freq = np.bincount(data_generator.labels_test)
        print('Average Accuracy: {:.4f}'.format(
            ((test_pred == np.asarray(data_generator.labels_test)).astype(np.float) / class_freq[np.asarray(data_generator.labels_test)]).sum() / len(
                class_freq)
        ))
